 ...
 * Color wheel spread: green, indigo, violet, blue, pink, orange, amber, rose, yellow, zinc, gray
...
export const EVENT_COLORS: Record<EventColor, { bg: string; text: string; border: string }> = {
  ...
  green:   { bg: "#def7e4", text: "#15803d", border: "#bbf7d0" },
  orange:  { bg: "#fff3e0", text: "#e65c00", border: "#fed7aa" },
  ...
  cyan:    { bg: "#e0f9fe", text: "#0891b2", border: "#a5f3fc" },
...
// ── Status → color key mapping (calendar pills) ─────────────────────────────
// Each lives on a different part of the color wheel for instant distinction:
//   green    = success
//   indigo   = scheduled (future, calm)
//   blue     = running (active)
//   pink     = auto-retrying (action, warm)
//   orange   = stale recovery (warning, warm)
//   amber    = needs retry (warning)
//   violet   = queued (purple, waiting)
//   rose     = failed (error)
//   zinc     = cancelled (dismissed)
//   yellow   = skipped
//   gray     = draft
...
// Same color semantics as above, mapped to Tailwind utility classes.
...
export const STATUS_BADGE_MAP: Record<string, { label: string; className: string; tooltip: string }> = {
  ...
  failed:         { label: "✗ Failed",       className: "border-rose-500/40 bg-rose-500/10 text-rose-600 dark:text-rose-400", tooltip: "The run failed — check output for errors" },
  running:        { label: "● Running",      className: "border-orange-500/40 bg-orange-500/10 text-orange-600 dark:text-orange-400", tooltip: "Currently executing" },
...
export const STATUS_GUIDE_ENTRIES: ReadonlyArray<{
  ...
  },
  {
    key: "queued",
    label: "Queued",
    desc: "Cron job assigned in the gateway — waiting to fire.",
  ...
  // ── Success ──
  {
    key: "succeeded",
    label: "Succeeded",
    desc: "Task completed successfully. Output and artifacts available.",
  ...
  // ── Terminal failures ──
  {
    key: "failed",
    label: "Failed",
    desc: "All retry attempts exhausted — terminal failure.",
  ...
  },
  {
    key: "needs_retry",
    label: "Needs Retry",
    desc: "Run failed. Manual retry required.",
  ...
  // ── Retry / recovery states ──
  {
    key: "auto_retry",
    label: "Auto-retrying",
    desc: "Automatically retrying with a fallback model.",
    colorKey: "pink" as EventColor,
  ...
  },
  {
    ...
    bg: "bg-gray-400/8 dark:bg-gray-400/10",
