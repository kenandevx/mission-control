...
export function normalizeWorkerSettings(input: Partial<WorkerSettings> | null | undefined): WorkerSettings {
...
export function isScheduleReady(scheduledFor: string | null, now = new Date()): boolean {
...
export function isPickupEligible(
  ticket: TicketRow,
  inProgressColumnIds: Set<string>,
  now = new Date(),
): boolean {
...
export function capacityLeft(maxConcurrency: number, currentlyExecuting: number): number {
  const cap = Math.max(0, Math.floor(maxConcurrency) - Math.max(0, Math.floor(currentlyExecuting)));
  return cap;
}
