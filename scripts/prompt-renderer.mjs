...
export function renderUnifiedTaskMessage({ title, context, request, instructions, artifactDir, isMainSession }) {
  ...
  // Execution rules and output rules are internal framework instructions.
  // When isMainSession is true, the user will see these in their Telegram chat,
  // so omit them entirely. The agent already knows how to behave; these rules
  // only exist to shape isolated-session output that bridge-logger captures.
  if (!isMainSession) {
    ...
    // Main session: just the artifact path if available
    const ad = clean(artifactDir);
    if (ad) {
      const artifactNote = `If you create any output files, save them to: ${ad} (unless the request specifies a different path).`;
      sections.push(artifactNote);
    }
  }
